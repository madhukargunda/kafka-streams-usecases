...
import io.madhu.orderProcessor.handlers.OrderStreamDeserializationExceptionHandler;
import io.madhu.orderProcessor.handlers.OrderStreamProcessorCustomExceptionHandler;
import io.madhu.orderProcessor.model.Order;
import io.madhu.orderProcessor.model.OrderType;
import io.madhu.orderProcessor.serdes.factory.SerdesFactory;
...
public class OrderKafkaStreamsConfiguration {
    ...
    public NewTopic generalOrders() {
        return TopicBuilder.name(RESTAURANT_ORDERS_TOPIC)
                .partitions(1)
                .build();
    }
    ...
    public NewTopic restaurantOrders() {
        return TopicBuilder.name(GENERAL_ORDERS_TOPIC)
                .partitions(1)
                .build();
    }
    ...
    public Topology ordersTopology() {
        ...
        KStream<String, Order> orderStreams = orderStreamBuilder
                .stream(ORDERS_TOPIC, Consumed.with(Serdes.String(),
       ...
       // orderStreams.print(Printed.<String, Order>toSysOut().withLabel("Orders-"));
        ...
        // Flatten the list of orders into individual orders
        // KStream<String, Order> flattenedStream = kstream.flatMapValues((key, listOfOrders) -> listOfOrders);

        //Print the flattened stream for debugging

        //   kstream.print(Printed.<String, Order>toSysOut().withLabel("flattened-Orders"));
        ...
        //extracted(orderStreams, generalOrdersPredicate, resturantOrdersPredicate);

        Map<String, KStream<String, Order>> branches = orderStreams.split(Named.as("split-stream-"))
                .branch((key, order) -> order.orderType().name().equals(OrderType.GENERAL.name()), Branched.as("general-orders"))
                .branch((key, order) -> order.orderType().name().equals(OrderType.RESTAURANT.name()), Branched.as("restaurant-orders"))
                .noDefaultBranch();
...
//        }
    ...
    private static void extracted(KStream<String, Order> orderStreams, Predicate<String, Order> generalOrdersPredicate, Predicate<String, Order> resturantOrdersPredicate) {
        orderStreams.split(Named.as("split-stream-"))
                .branch(generalOrdersPredicate, Branched.withConsumer(generalOrdersStream -> {
                    generalOrdersStream.print(Printed.<String, Order>toSysOut().withLabel("general-order-stream"));
                    generalOrdersStream.to(GENERAL_ORDERS_TOPIC, Produced.with(Serdes.String(), SerdesFactory.orderSerde()));
                }))
   ...
   /* @Bean
    public KafkaStreams kafkaStreams1() {
        Map<String, Object> props = kafkaStreamProperties.toMap();
        props.put(StreamsConfig.APPLICATION_ID_CONFIG, kafkaStreamProperties.getApplicationId() + "-1");

        StreamsBuilder streamsBuilder1 = ordersStreamBuilder();
        Topology topology1 = ordersTopology(streamsBuilder1);
        KafkaStreams streams = new KafkaStreams(topology1, props);

        if (kafkaStreamsProperties.isCleanupOnStart()) {
            streams.cleanUp();
        }
